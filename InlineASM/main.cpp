...
__declspec(dllexport) wchar_t* ASMResult(int* cypher, char* clen, int ccount, wchar_t* output);
...
int main() {
	...
	int* cypherPtr = cypher;
	char* clenPtr = clen;
	wchar_t *outp = output;


	wchar_t* ptr = ASMResult(cypherPtr, clenPtr, ccount, outp);
